...
class Database:
    def __init__(self, db_path: Optional[str] = None):
        if db_path is None:
            if os.path.exists("gemini_bot.db") and not os.path.exists("agy_bot.db"):
                self.db_path = "gemini_bot.db"
            else:
                self.db_path = "agy_bot.db"
        else:
    ...
    def _init_db(self):
        with self._get_connection() as conn:
            with conn:
                ...
                # Sessions table (mapping chat_jid to AGY conversation UUIDs)
                cursor.execute("""
